...
从 [Releases](https://github.com/bGlzdGRlcg/rssbot/releases) 页面下载对应的版本解压运行即可。
...
wget -O ~/flowerss/config.yml \
    https://raw.githubusercontent.com/bGlzdGRlcg/rssbot/master/config.yml.sample
```
...
```shell script
docker run -d -v ~/flowerss:/root/.flowerss bGlzdGRlcg/rssbot
```

## 源码编译部署

```shell script
git clone https://github.com/bGlzdGRlcg/rssbot && cd flowerss-bot
make build
./flowerss-bot
```
